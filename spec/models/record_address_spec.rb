  ...
  describe 'ユーザーの商品購入機能' do
    ...
    context '商品購入がうまくいかない時' do
      ...
      it "zip_codeが半角のハイフンを含んだ正しい形式でないと保存できないこと" do
        ...
        @record_address.valid?
        expect(@record_address.errors.full_messages).to include("郵便番号にはハイフンありの7桁である必要があること")
      end
      it "urban_idが空だと購入できない" do
        @record_address.urban_id = nil
        @record_address.valid?
        expect(@record_address.errors.full_messages).to include("都道府県を入力してください")
      end
      it "cityが空だと購入できない" do
        @record_address.city = nil
        @record_address.valid?
        expect(@record_address.errors.full_messages).to include("市町村区を入力してください")
      end
